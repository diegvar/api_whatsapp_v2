...
export class WhatsAppService {
    ...
    private readonly HEALTH_CHECK_INTERVAL_MS = 2 * 60 * 1000; // cada 2 minutos
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const messageId =
                    ...
                    (response.id as { $1?: string } | undefined)?.$1 ||
                    undefined;
                ...
                const isProtocolTimeout =
                    lastError.message.includes('Runtime.callFunctionOn timed out') ||
                    lastError.message.includes('protocolTimeout') ||
                    lastError.name === 'ProtocolError' ||
                    (lastError as Error & { name?: string }).constructor?.name === 'ProtocolError';

                const isDetachedFrame =
                    lastError.message.includes('detached Frame') ||
                ...
                const isEmptyResponse =
                    lastError.message.includes('respuesta vacía') ||
                ...
                const isClientInitError =
                    lastError.message.includes('getChat') ||
    ...
    private startFrameHealthCheck(): void {
        this.stopFrameHealthCheck();
    ...
    private stopFrameHealthCheck(): void {
    ...
    private async isPuppeteerFrameAlive(): Promise<boolean> {
        try {
            if (!this.client?.pupPage) {
                return false;
            }
            const result = await this.client.pupPage.evaluate(() => {
                return typeof (globalThis as { WWebJS?: unknown }).WWebJS !== 'undefined';
            });
            return result === true;
        } catch (error) {
    ...
    private async runFrameHealthCheck(): Promise<void> {
